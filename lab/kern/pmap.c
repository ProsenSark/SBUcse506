...
i386_detect_memory(void)
{
	...
	clog("base = %d pages, ext = %d pages", basemem/PGSIZE, extmem/PGSIZE);
}
...
boot_alloc(uint32_t n, uint32_t align)
{
	...
	if (!is_power_of_two(align)) {
		cprintf("boot_alloc: align = %u is not a power of 2, failed allocation!\n", align);
		return NULL;
	}
	v = boot_freemem;
	boot_freemem = ROUNDUP(boot_freemem, align);
	uintptr_t va = (uintptr_t) boot_freemem;
	if (va >= (MAXVAL(va) - 1 - n)) {
		boot_freemem = v;
...
i386_vm_init(void)
{
	...
	assert(pages != NULL);
	clog("pages = %p, sizeof(struct Page) = %u, boot_freemem = %p",
			pages, sizeof(struct Page), boot_freemem);
	...
	assert(envs != NULL);
	clog("envs = %p, sizeof(struct Env) = %u", envs, sizeof(struct Env));
	...
	clog("KSTACKTOP - KSTKSIZE = %p", KSTACKTOP - KSTKSIZE);
	boot_map_segment(pgdir, KSTACKTOP - KSTKSIZE, n, PADDR(bootstack),
			PTE_W | PTE_P);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	n = ROUNDUP(MAXVAL(uintptr_t) - KERNBASE, PGSIZE);
	clog("MAXVAL(uintptr_t) = %p, MAXVAL(intptr_t) = %p",
			MAXVAL(uintptr_t), MAXVAL(intptr_t));
	boot_map_segment(pgdir, KERNBASE, n, 0, PTE_W | PTE_P);
	//clog("MAXVAL(uint64_t) = 0x%llx, MAXVAL(int64_t) = 0x%llx",
	//		MAXVAL(uint64_t), MAXVAL(int64_t));
...
page_init(void)
{
	...
	int i;
	LIST_INIT(&page_free_list);
	clog("PPN(IOPHYSMEM) = %d, PPN(EXTPHYSMEM) = %d, PPN(PADDR(boot_freemem)) = %d",
			PPN(IOPHYSMEM), PPN(EXTPHYSMEM), PPN(PADDR(boot_freemem)));
	for (i = 0; i < npage; i++) {
		pages[i].pp_ref = 0;
		if (i == 0) continue;
		if ( (PPN(IOPHYSMEM) <= i) && (i < PPN(EXTPHYSMEM)) ) {
			//clog("skipped PPN %d", i);
			continue;
		}
		if ( (PPN(EXTPHYSMEM) <= i) && (i <= PPN(PADDR(boot_freemem))) ) {
			//clog("skipped PPN %d", i);
			continue;
		}
...
page_alloc(struct Page **pp_store)
{
	...
	pf = LIST_FIRST(&page_free_list);
	if (pf != NULL) {
		LIST_REMOVE(pf, pp_link);
		page_initpp(pf);
		*pp_store = pf;
		return 0;
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if ((pgde & PTE_P) == 0) {
		...
		rc = page_alloc(&pp);
		if (rc != 0) {
			cprintf("pgdir_walk: Failed to allocate new page!\n");
			return NULL;
		}
		//clog("va = %p, PDX(va) = %u", va, PDX(va));
		pp->pp_ref = 1;
		ppa = page2pa(pp);
		//clog("pp = %p, KADDR(ppa) = %p", pp, KADDR(ppa));
		memset(KADDR(ppa), 0, PGSIZE);
		//pgde = ppa | PTE_USER;
		pgde = ppa | PTE_W | PTE_U | PTE_P;
		pgdir[PDX(va)] = pgde;
	}
	pgtbl = (pte_t*) KADDR(PTE_ADDR(pgde));
	//clog("pgtbl = %p", pgtbl);
	return &pgtbl[PTX(va)];
	//return NULL;
}
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	while ((size / PGSIZE) > 0) {
		...
		//assert(ppte != NULL);
		if (ppte == NULL) {
			panic("boot_map_segment: Error! No PTE for %p ?", va);
		}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if (pp->pp_ref == 0) {
		panic("page_remove: Error! duplicate request for remove?");
	}
	page_decref(pp);
	if (ppte != NULL) {
		*ppte = 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here. 
	uintptr_t va_end;
	uint32_t pdeno, pteno, ptemin, ptemax;
	pte_t* ppte = NULL;
	physaddr_t pa;

	assert(env != NULL);
	va_end = (uintptr_t) (va + len - 1);
	//clog("va = %p, va_end = %p", va, va_end);
	...
	if (va_end > ULIM) {
		user_mem_check_addr = ULIM + 1;
		//clog("wp2");
		return -E_FAULT;
	}

	for (pdeno = PDX(va); pdeno <= PDX(va_end); ++pdeno) {
		if (!(env->env_pgdir[pdeno] & (perm | PTE_P))) {
			user_mem_check_addr = (uintptr_t) PGADDR(pdeno, 0, 0);
			if (pdeno == PDX(va)) {
		...
		pa = PTE_ADDR(env->env_pgdir[pdeno]);
		ppte = (pte_t*) KADDR(pa);
		for (pteno = ptemin; pteno <= ptemax; ++pteno) {
			if (!(ppte[pteno] & (perm | PTE_P))) {
				user_mem_check_addr = (uintptr_t) PGADDR(pdeno, pteno, 0);
				if ( (pdeno == PDX(va)) && (pteno == ptemin) ) {
					// First Page Table in First Page Directory
					user_mem_check_addr = (uintptr_t) va;
				}
				//clog("wp4");
				return -E_FAULT;
...
page_check(void)
{
	...
	page_free(pp0);
    clog("boot_pgdir = %p, pp0 = %p", boot_pgdir, pp0);
	assert(page_insert(boot_pgdir, pp1, 0x0, 0) == 0);
    clog("boot_pgdir[0] = %p, page2pa(pp0) = %p", (boot_pgdir[0]), page2pa(pp0));
	assert(PTE_ADDR(boot_pgdir[0]) == page2pa(pp0));
	assert(check_va2pa(boot_pgdir, 0x0) == page2pa(pp1));
    clog("pp1->pp_ref = %d, pp0->pp_ref = %d", pp1->pp_ref, pp0->pp_ref);
	assert(pp1->pp_ref == 1);
	assert(pp0->pp_ref == 1);
