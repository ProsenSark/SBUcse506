...
#define ENVGENSHIFT	12		// >= LOG2NENV
...
env_init(void)
{
	...
	for (envid = NENV - 1; envid >= 0; --envid) {
		envs[ENVX(envid)].env_status = ENV_FREE;
		envs[ENVX(envid)].env_id = 0;
		LIST_INSERT_HEAD(&env_free_list, &envs[ENVX(envid)], env_link);
	}
	clog("USTACKTOP = %p, USTACKTOP - PGSIZE = %p", USTACKTOP,
			USTACKTOP - PGSIZE);
	clog("UXSTACKTOP = %p, UXSTACKTOP - PGSIZE = %p", UXSTACKTOP,
			UXSTACKTOP - PGSIZE);
}
...
segment_alloc(struct Env *e, void *va, size_t len)
{
	...
	for ( ; va < va_end; va += PGSIZE) {
		rc = page_alloc(&pp);
		if (rc != 0) {
			panic("segment_alloc: page_alloc FAILED: %e", rc);
		}
		rc = page_insert(e->env_pgdir, pp, va, PTE_W | PTE_U);
		if (rc != 0) {
			panic("segment_alloc: page_insert FAILED: %e", rc);
		}
	}
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	assert(e != NULL);
	assert(binary != NULL);
	pelf = (struct Elf *) binary;
	if (pelf->e_magic != ELF_MAGIC) {
		panic("load_icode: Not an ELF image?");
	}
	ph = (struct Proghdr *) ((uint8_t *) pelf + pelf->e_phoff);
	eph = ph + pelf->e_phnum;
	for ( ; ph < eph; ++ph) {
		...
		va = (void *) ph->p_va;
		segment_alloc(e, va, ph->p_memsz);
		//clog("wp1: va = %p, src = %p, size = %u", va,
		//		binary + ph->p_offset, ph->p_filesz);

		// Following 4 lines is akin to 'copy_to_user' in Linux kernel API
		lcr3(e->env_cr3);
		memmove(va, binary + ph->p_offset, ph->p_filesz);
		memset(va + ph->p_filesz, 0, ph->p_memsz - ph->p_filesz);
		//lcr3(boot_cr3);
	}
	e->env_tf.tf_eip = pelf->e_entry;
...
env_create(uint8_t *binary, size_t size)
{
	...
	rc = env_alloc(&penv, 0);
	if (rc != 0) {
		panic("env_create: env_alloc FAILED: %e", rc);
	}
