...
static int
send_data(struct http_request *req, int fd, off_t fsize)
{
...
static const char*
file_ext(const char *file)
{
	int n;

	for (n = (strlen(file) - 1); n > 0; --n) {
		if (file[n] == '.') {
			return (file + n + 1);
		}
	}
